...
block = SoftLayer.BlockStorageManager(client)

ids = [volume['id'] for volume in block.list_block_volumes()]
...
if input() == 'delete':
    for id in volumes_to_delete:
        try:
            block.cancel_block_volume(id, f'automated cleanup for cluster {cluster_id}', immediate=True)
